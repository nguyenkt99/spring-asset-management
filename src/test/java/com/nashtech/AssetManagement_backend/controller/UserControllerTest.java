...
public class UserControllerTest {
    ...
    public void setUp() {
        ...
        userDto.setType(RoleName.ROLE_STAFF);
...
//    }
    ...
    @Test
    void canDisableUser_then_return_true() throws Exception{
        Mockito.when(userService.canDisableUser(ArgumentMatchers.anyString())).thenReturn(true);
        mockMvc.perform(get("/api/users/disable/staffCode").contentType(MediaType.APPLICATION_JSON)
        .characterEncoding("utf-8").accept(MediaType.APPLICATION_JSON)).andExpect(status().isOk())
                .andExpect(content().string(containsString("true")));
    }

    @Test
    void disableUser_then_return_true() throws Exception{
        UserDto userDto = new UserDto();
        userDto.setStaffCode("staffCode");
        userDto.setState(UserState.Disabled);
        Mockito.when(userService.disableUser(ArgumentMatchers.anyString())).thenReturn(true);
        mockMvc.perform(post("/api/disable/"+ userDto.getStaffCode()).contentType(MediaType.APPLICATION_JSON).characterEncoding("utf-8").accept(MediaType.APPLICATION_JSON)).andExpect(status().isOk())
                .andExpect(jsonPath("$.state",Matchers.equalTo(userDto.getState())))
                .andExpect(content().string(containsString("true")));
    }
